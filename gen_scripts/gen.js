...
*/
    ...
    function getReqs(serviceDef) {
      reqs = {};
      Object.keys(serviceDef.operations)
        .map((op) => {
          if (
            serviceDef.operations[op].input &&
            serviceDef.operations[op].input.required
          ) {
    ...
    serviceDef.metadata.serviceName = serviceNameMapper(
      serviceDef.metadata.endpointPrefix
    );
    var htmlFile = `
<!--
...
<script type="text/x-red" data-template-name="AWS ${
      serviceDef.metadata.serviceName
    }">
		<style scoped>
        ...
        (op) => `
            ...
            serviceDef.operations[op].input
              ? mapKeys(
                  serviceDef.operations[op].input.required,
                  (reqIdx) =>
                    `'#${serviceDef.operations[op].input.required[reqIdx]}Attr'`,
                  ','
                )
              : ''
          }
...
<script type="text/x-red" data-help-name="amazon ${
      serviceDef.metadata.serviceName
    }">
<p>
...
module.exports = function(RED) {
	...
	function AmazonAPINode(n) {
		...
		var awsService = new AWS.${serviceDef.metadata.serviceName}( constructor_args );
      ...
      (op) => `
        ...
        serviceDef.operations[op].input.required
          ? mapKeys(
              ...
              (input) => `
			copyArg(n,"${
        serviceDef.operations[op].input.required[input]
      }",params,undefined,${
                typeof serviceDef.operations[op].input.members[
                  serviceDef.operations[op].input.required[input]
                ].shape !== 'undefined'
              }); `
            )
          : ''
			...
			${
        serviceDef.operations[op].input &&
        serviceDef.operations[op].input.members
          ? mapKeys(
              serviceDef.operations[op].input.members,
              (input) => `
			copyArg(msg,"${input}",params,undefined,${
                typeof serviceDef.operations[op].input.members[input].shape !==
                'undefined'
              }); `
            )
          : ''
			...
			${
        serviceDef.operations[op].input && serviceDef.operations[op].input.shape
          ? mapKeys(
              serviceDef.shapes[serviceDef.operations[op].input.shape].members,
              (input) => `
